...
public final class CustomSingleSignOutHandler {
    ...
    public boolean process(final HttpServletRequest request, final HttpServletResponse response) {
        if (isTokenRequest(request)) {
            logger.info("Received a token request");
            recordSession(request);
            return true;

        } else if (isBackChannelLogoutRequest(request)) {
            logger.info("Received a back channel logout request");
            destroySession(request);
            return false;

        } else if (isFrontChannelLogoutRequest(request)) {
            logger.info("Received a front channel logout request");
            destroySession(request);
            ...
            logger.info("Ignoring URI for logout: {}", request.getRequestURI());
            return true;
        }
    }
    ...
    private void recordSession(final HttpServletRequest request) {
        ...
        if (session == null) {
            logger.info("No session currently exists (and none created).  Cannot record session information for single sign out.");
            return;
        }

        final String token = CommonUtils.safeGetParameter(request, this.artifactParameterName, this.safeParameters);
        logger.info("Recording session for token {}", token);
    ...
    private String computeRedirectionToServer(final HttpServletRequest request) {
        ...
        if (StringUtils.isNotBlank(relayStateValue)) {
            ...
            logger.info("Redirection url to the CAS server: {}", redirectUrl);
            return redirectUrl;
        }
        return null;
    }
    ...
    private class Servlet30LogoutStrategy implements LogoutStrategy {
        ...
        public void logout(final HttpServletRequest request) {
            try {
                ...
                logger.info("Error performing request.logout.");
            }
        }
    }
}
